...
if st.button("开始云端求解"):
    ...
    try:
        ...
        if rmp.status == GRB.OPTIMAL:
            # --- 利用率计算 ---
            total_obj = int(rmp.ObjVal + 0.5)
            total_parts_length = sum(wi * bi for wi, bi in zip(w, b))
            total_material_length = total_obj * L
            utilization_rate = (total_parts_length / total_material_length) * 100

            col_res1, col_res2 = st.columns(2)
            col_res1.metric("母料总需求量", f"{total_obj} 根")
            col_res2.metric("原材料总利用率", f"{round(utilization_rate, 2)}%")
